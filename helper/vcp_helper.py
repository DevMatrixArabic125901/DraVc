    ...
    async def join_vc(self, chat, join_as=None):
        ...
        try:
            await self.app.join_group_call(
                chat_id=chat.id,
                stream=AudioPiped("dragovc/resources/Silence01s.mp3"),
            ...
            return "- عليك تثبيت المتطلبات اولا شاهاد القناة الاساسية @drago"
        ...
        if not self.PLAYLIST:
            if self.PLAYING:
                await self.app.change_stream(
                    self.CHAT_ID,
                    AudioPiped("dragovc/resources/Silence01s.mp3"),
                )
            self.PLAYING = False
